...
func (m *Manager) ClearSessionByUserID(ctx context.Context, userID string) error {
	return m.tokenStore.ClearSessionByUserID(ctx, userID)
}
...
func (m *Manager) RefreshAccessToken(ctx context.Context, tgr *oauth2.TokenGenerateRequest) (oauth2.TokenInfo, error) {
	...
	if rcfg.IsRemoveAccess {
		...
		m.tokenStore.RemoveByAccess(ctx, oldAccess)
	}

	if rcfg.IsRemoveRefreshing && rv != "" {
		// remove the old refresh token
		m.tokenStore.RemoveByRefresh(ctx, oldRefresh)
	}
