...
// ClearSessionByUserID implements oauth2.TokenStore.
func (ts *TokenStore) ClearSessionByUserID(ctx context.Context, userID string) error {
	return ts.db.View(func(tx *buntdb.Tx) error {
		jv, err := tx.Get(userID)
		if err != nil {
			return err
		}

		var keys []string
		err = json.Unmarshal([]byte(jv), &keys)
		if err != nil {
			return err
		}
...
func (ts *TokenStore) Create(ctx context.Context, info oauth2.TokenInfo) error {
	...
	return ts.db.Update(func(tx *buntdb.Tx) error {
		...
		sessions := []string{}
		userSessionString, err := tx.Get(info.GetUserID())
		if err == nil {
			json.Unmarshal([]byte(userSessionString), &sessions)
		}
		...
		// Add to sessions
		sessions = append(sessions, basicID)
		sessions = append(sessions, info.GetAccess())
		sessions = append(sessions, info.GetRefresh())

		sessionsByte, _ := json.Marshal(sessions)
		tx.Set(info.GetUserID(), string(sessionsByte), &buntdb.SetOptions{Expires: info.GetRefreshExpiresIn() != 0, TTL: info.GetRefreshExpiresIn()})
...
func (ts *TokenStore) RemoveByAccess(ctx context.Context, access string) error {
	ts.db.View(func(tx *buntdb.Tx) error {
		key, err := tx.Get(access)
		if err != nil {
			return err
		}
		...
		err = json.Unmarshal([]byte(jv), &info)
		if err != nil {
			return err
		}

		sessionsStr, err := tx.Get(info.GetUserID())
		if err != nil {
			return err
		}

		var sessions []string
		err = json.Unmarshal([]byte(sessionsStr), &sessions)
		if err != nil {
			return err
		}
