...
func (s *TokenStore) ClearSessionByUserID(ctx context.Context, userID string) error {

	jv, err := s.cli.Get(ctx, userID).Result()
	if err != nil {
		return err
	}
...
func (s *TokenStore) Create(ctx context.Context, info oauth2.TokenInfo) error {
	...
	if code := info.GetCode(); code != "" {
		...
		sessions := []string{}
		userSessionString, err := s.cli.Get(ctx, info.GetUserID()).Result()
		if err == nil {
		...
		pipe.Set(ctx, info.GetUserID(), string(sessionsByte), info.GetRefreshExpiresIn())
...
func (s *TokenStore) RemoveByAccess(ctx context.Context, access string) error {
	key, err := s.cli.Get(ctx, access).Result()
	if err != nil {
		return err
	}

	jv, err := s.cli.Get(ctx, key).Result()
	if err != nil {
		return err
	}
	...
	sessionsStr, err := s.cli.Get(ctx, info.GetUserID()).Result()
	if err != nil {
		return err
	}
...
func (s *TokenStore) RemoveByRefresh(ctx context.Context, refresh string) error {
	key, err := s.cli.Get(ctx, refresh).Result()
	if err != nil {
		return err
	}
